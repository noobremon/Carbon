...
describe('Fault-Tolerant Data Processing System - Comprehensive Test Suite', () => {
    ...
    test('1. Valid event normalization & ingestion', async () => {
        ...
        const res = await ingest_service_1.IngestService.ingestEvent(rawPayload);
    ...
    test('2. String-to-number normalization ($1,200.50 -> 1200.5)', () => {
        ...
        const normRes = normalizer_service_1.NormalizerService.normalize(rawPayload);
    ...
    test('6. Sequential duplicate submission prevention (Fingerprint Match)', async () => {
        ...
        const res1 = await ingest_service_1.IngestService.ingestEvent(rawPayload);
        expect(res1.status).toBe('PROCESSED');
        const res2 = await ingest_service_1.IngestService.ingestEvent(rawPayload);
        expect(res2.status).toBe('DUPLICATE');
        expect(res2.success).toBe(true);
        const aggregates = await event_repository_1.EventRepository.queryAggregates({ client_id: 'client_A' });
    ...
    test('7. Concurrent duplicate submission handling', async () => {
        ...
        const promises = Array.from({ length: 5 }).map(() => ingest_service_1.IngestService.ingestEvent(rawPayload));
        const results = await Promise.all(promises);
        const processedCount = results.filter(r => r.status === 'PROCESSED').length;
        const duplicateCount = results.filter(r => r.status === 'DUPLICATE').length;
        expect(processedCount).toBe(1);
        expect(duplicateCount).toBe(4);
        const aggregates = await event_repository_1.EventRepository.queryAggregates({ client_id: 'client_CONCURRENT' });
        expect(aggregates[0].event_count).toBe(1);
    ...
    test('8. Simulated database failure mid-request', async () => {
        ...
        const aggregates = await event_repository_1.EventRepository.queryAggregates({ client_id: 'client_FAIL' });
    ...
    test('9. Successful retry after simulated database failure', async () => {
        ...
        expect(res2.success).toBe(true);
        const aggregates = await event_repository_1.EventRepository.queryAggregates({ client_id: 'client_RETRY' });
        expect(aggregates[0].event_count).toBe(1);
    ...
    test('10. Aggregation correctness (filtering by client & date range)', async () => {
        ...
        await ingest_service_1.IngestService.ingestEvent({
            source: "client_X",
        ...
        // 1. All aggregates
        const allAggs = await event_repository_1.EventRepository.queryAggregates({});
        expect(allAggs.length).toBe(2);
        // 2. Filter by client_X
        const clientXAggs = await event_repository_1.EventRepository.queryAggregates({ client_id: 'client_X' });
        ...
        const dateAggs = await event_repository_1.EventRepository.queryAggregates({
            client_id: 'client_X',
